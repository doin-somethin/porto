...
export function ProfileCardComponent({ isDarkMode }) {
  ...
  return (
    <div 
      ...
      // PERBAIKAN 1: Ubah p-[1px] menjadi p-[2px] agar GARIS DASAR kartu lebih tebal dan solid di sudutnya
      className={`relative p-[2px] w-[92%] sm:w-[95%] max-w-[1300px] mx-auto rounded-[2.1rem] group transition-shadow duration-700 ease-in-out mt-4 md:mt-0 ${
        isDarkMode 
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(350px circle at ${mousePosition.x}px ${mousePosition.y}px, ${
            isDarkMode ? 'rgba(255,255,255,1)' : 'rgba(37,99,235,0.9)' 
          }, transparent 100%)`,
          // PERBAIKAN 2: Padding masking juga disamakan menjadi 2px agar cahayanya mengisi penuh ketebalan border
          padding: '2px',
      ...
      <div className={`relative h-full w-full rounded-[2rem] p-6 sm:p-8 md:p-14 flex flex-col-reverse md:flex-row items-center md:items-stretch justify-between gap-8 md:gap-12 lg:gap-20 overflow-hidden z-10 backdrop-blur-2xl transition-colors duration-700 ${
        ...
        <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left z-20 w-full max-w-2xl py-2">
          <div className="w-full">
            <h3 className={`text-sm sm:text-base md:text-xl font-medium mb-3 md:mb-4 tracking-wide transition-colors duration-700 ${
              isDarkMode ? 'text-gray-300' : 'text-gray-600'
            }`}>
              Hello, I'm Yosea Mervandy Sugiarto,
            </h3>
            
            <h1 className={`text-4xl sm:text-5xl md:text-7xl lg:text-[5.5rem] font-bold tracking-tight mb-6 md:mb-8 transition-colors duration-700 leading-[1.15] md:leading-[1.1] ${
            ...
            <p className={`max-w-[280px] sm:max-w-sm md:max-w-lg mx-auto md:mx-0 text-xs sm:text-sm md:text-[17px] leading-relaxed mb-8 md:mb-0 transition-colors duration-700 font-light ${
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-6 md:gap-8 mt-6 md:mt-auto pb-2">
            <button className={`px-8 py-3 md:px-9 md:py-3.5 rounded-full font-medium transition-all duration-300 shadow-lg hover:-translate-y-1 text-sm md:text-base tracking-wide z-20 relative ${
              isDarkMode 
            ...
            <div className="flex items-center gap-5 md:gap-6 ml-0 md:ml-4 z-20 relative">
              ...
              <a href="https://github.com/YoseaMerv" target="_blank" rel="noopener noreferrer" className={`transition-all duration-300 hover:-translate-y-1 ${
                ...
                <Github size={22} className="md:w-6 md:h-6" strokeWidth={1.5} />
              </a>
              <a href="https://www.instagram.com/yoseamerv_2/" target="_blank" rel="noopener noreferrer" className={`transition-all duration-300 hover:-translate-y-1 ${
                ...
                <Instagram size={22} className="md:w-6 md:h-6" strokeWidth={1.5} />
              </a>
            </div>
          </div>
        </div>
        ...
        {/* --- KOLOM KANAN: GAMBAR --- */}
        <div className={`w-full max-w-[220px] sm:max-w-[280px] md:max-w-[400px] lg:max-w-[440px] aspect-square md:aspect-[4/5] rounded-[1.5rem] overflow-hidden shadow-2xl transition-transform duration-700 group-hover:scale-[1.02] flex-shrink-0 relative z-20 ${
